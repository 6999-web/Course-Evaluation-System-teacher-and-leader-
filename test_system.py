...
def test_teacher_materials():
    """测试教师端材料管理"""
    if not teacher_token:
        result.add_fail("教师端材料管理", "未登录")
        return False
    
    # 尝试POST请求
    url = f"{TEACHER_API_BASE}/teacher/materials/sync"
    data = {"sync_data": {}}
    response = make_request("POST", url, token=teacher_token, data=data)
    if response:
        result.add_pass("教师端材料管理")
        return True
    else:
        result.add_fail("教师端材料管理", "获取数据失败")
        return False
...
def test_admin_login():
    ...
    if response and "token" in response:
        ...
        result.add_fail("管理端登录", f"无法获取token: {response}")
        return False
...
def test_admin_tasks():
    ...
    url = f"{ADMIN_API_BASE}/evaluation-tasks"
...
def test_admin_scoring_config():
    """测试管理端评分规则配置"""
    if not admin_token:
        result.add_fail("管理端评分规则配置", "未登录")
        return False
    
    url = f"{ADMIN_API_BASE}/scoring-config"
    response = make_request("GET", url, token=admin_token)
    if response:
        result.add_pass("管理端评分规则配置")
        return True
    else:
        result.add_fail("管理端评分规则配置", "获取数据失败")
        return False

def test_admin_materials():
    """测试管理端材料管理"""
    if not admin_token:
        result.add_fail("管理端材料管理", "未登录")
        return False
    
    url = f"{ADMIN_API_BASE}/materials"
    response = make_request("GET", url, token=admin_token)
    if response:
        result.add_pass("管理端材料管理")
        return True
    else:
        result.add_fail("管理端材料管理", "获取数据失败")
        return False

def test_admin_scoring():
    """测试管理端自动评分"""
    if not admin_token:
        result.add_fail("管理端自动评分", "未登录")
        return False
    
    url = f"{ADMIN_API_BASE}/scoring/auto-score"
    data = {
        "material_id": "test_material_001",
        "file_type": "教案"
    }
    response = make_request("POST", url, token=admin_token, data=data)
    if response:
        result.add_pass("管理端自动评分")
        return True
    else:
        result.add_fail("管理端自动评分", "评分失败")
        return False
...
def run_tests():
    ...
    if test_admin_login():
        test_admin_tasks()
        test_admin_scoring_config()
        test_admin_materials()
        test_admin_scoring()
